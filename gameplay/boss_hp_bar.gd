...
func _ready():
	...
	if has_node("ProgressBar"):
		print("BossHPBar: ProgressBar found")
		update_hp(current_hp)
	else:
		print("BossHPBar: ERROR - ProgressBar not found!")
...
func update_hp(value):
	...
	print("BossHPBar update_hp called: ", current_hp, "/", max_hp)
	
	# อัปเดต ProgressBar
	if has_node("ProgressBar"):
		$ProgressBar.value = current_hp
		print("BossHPBar ProgressBar value set to: ", $ProgressBar.value)
	else:
		print("BossHPBar ERROR: ProgressBar not found in update_hp")
